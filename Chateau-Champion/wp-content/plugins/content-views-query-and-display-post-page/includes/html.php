		...
		static function _field_content( $post, $fargs ) {
			...
			switch ( $fargs[ 'content' ][ 'show' ] ) {
				case 'excerpt':
					...
					$tail			 = $show_dots ? ' ...' : '';
					// DC 10 mars 2017 - détection du champ Extrait"
					// $resume = get_post_meta(get_the_ID(), "résumé", true);
					...
					if ( apply_filters( PT_CV_PREFIX_ . 'field_content_readmore_enable', 1, $fargs[ 'content' ] ) ) {
						...
						/* DC 3 avril 2017 - add svg arrow */
						$svgArrow = twentyseventeen_get_svg( array( 'icon' => 'arrow-right' ) );

						$btn_class		 = PT_CV_PREFIX . 'readmore ' . apply_filters( PT_CV_PREFIX_ . 'field_content_readmore_class', 'btn btn-success', $fargs );
						$readmore_btn	 = self::_field_href( $post, $svgArrow  . $readmore_text, $btn_class );
						$tail .= apply_filters( PT_CV_PREFIX_ . 'field_content_readmore_seperated', '<br/>', $fargs );
					}

					// Get excerpt
					// DC 10 mars 2017 - Test "Extrait"
					if ($resume) {	
						$content = "<div class='resume'>".$resume ."</div>". $readmore_btn;
					} else {
						if ( $length > 0 ) {
