...
function vsel_all_events_shortcode( $vsel_atts ) {
	$vsel_atts = shortcode_atts( array( 
		...
		'date_label' => __('Le %s', 'very-simple-event-list'), // date label
		'start_label' => __('Du %s', 'very-simple-event-list'), // start date label
				...
				/*
					...
					*/
					...
					if (!empty($event_location)){
						...
						$output .= twentyseventeen_get_svg( array( 'icon' => 'location' ) ) .esc_attr($event_location) ; 
						$output .= '</p>';
					}
					if (!empty($event_link)){
						if ($widget_link == 'yes') {
