...
@-webkit-keyframes n2InfiniteWhite {
    ...
    public function renderSlider($slider, $sliderHTML) {
        ...
        if (!empty($customSpinner)) {
            ...
            $marginLeft = -($width / 2);
            $marginTop  = -($height / 2);
            return $sliderHTML . '<div id="' . $slider->elementId . '-spinner"><img src="' . N2ImageHelper::fixed($customSpinner) . '" style="width:' . $width . 'px; height:' . $height . 'px; position:absolute;left:50%;top:50%;margin-left:' . $marginLeft . 'px;margin-top:' . $marginTop . 'px;" alt="loading"/></div>';
